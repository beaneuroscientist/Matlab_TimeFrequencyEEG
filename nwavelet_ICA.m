function EEG=nwavelet_ICA(EEG,varargin)
%% computes wavelet analysis on ICA components of EEG data
...
%% Select Components to compute wavelet on
[t, ~]=size(EEG.icaweights);
if isempty(EEG.icaact)
EEG.icaact = (EEG.icaweights*EEG.icasphere)*EEG.data(EEG.icachansind,:);

EEG.icaact=reshape(EEG.icaact,t,EEG.pnts,EEG.trials);
end
if inps.Comps==0
    inps.Comps=1:t;
end
chidx=inps.Comps;
%% Define the wavelet and compute will be the same across components
...
%% Compute the Wavelet iterating over all components.
%Creates a wavelet variable used for all components iterations
...
%% compute baseline index wavelet decomposition on each Component
...
for ch=1:length(chidx)
    ...
    eegfft(ch,:) = fft(reshape(data(ch,:,:),1,EEG.pnts*EEG.trials),n_conv_pow2);
...
EEG.icawavelet.frequencies=frex;
EEG.icawavelet.power=eegpower;
EEG.icawavelet.itpc=itpc;
EEG.icawavelet.cycles=s;


if strcmp(inps.RawRetain,'on')
    EEG.icawavelet.raw=raw;
end
end